...
def mark_for_add(evt: gr.SelectData) -> Optional[str]:
    if evt.value and isinstance(evt.value, dict) and 'image' in evt.value and 'path' in evt.value['image']:
        return evt.value['image']['path']
    return None

def mark_for_remove(evt: gr.SelectData) -> Optional[str]:
    if evt.value and isinstance(evt.value, dict) and 'image' in evt.value and 'path' in evt.value['image']:
        return evt.value['image']['path']
    return None

def add_marked_to_selected(marked_path: str, current_selected: List[str]) -> List[str]:
    if not marked_path:
...
def remove_marked_from_selected(marked_path: str, current_selected: List[str]) -> List[str]:
...
def save_prompt_to_db(title: str, content: str) -> gr.Dropdown:
...
def delete_prompt_from_db(selected_title: str) -> gr.Dropdown:
    if not selected_title or selected_title == i18n.get("home_control_prompt_placeholder"):
        return gr.skip()
...
def render() -> Dict[str, gr.Component]:
    ...
    with gr.Row(equal_height=False):
        with gr.Column(scale=4):
            # 渲染素材库块
            assets_ui = assets_block.render_assets_block(prefix="main_")
        ...
        with gr.Column(scale=6):
            with gr.Group():
                ...
                gallery_selected = gr.Gallery(label=i18n.get("home_control_gallery_selected_label"), elem_id="fixed_gallery", height=240, columns=6, rows=1, show_label=False, object_fit="contain", interactive=False)
